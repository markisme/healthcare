...
(def-category N
  (visible-atts (move wh plu per 3sg genitive nform adv case))
...
(def-category N_C
  (visible-atts (move wh plu per 3sg genitive nform adv case ct pron lexitem))
  (subcat-atts (genitive adv))
  )
...
(def-category A
  ...
  (conj-atts (move wh) (move wh adv nform att))
  )
...
(def-category C
  (visible-atts (move bare cform tense comp))
...
(def-modifiers N
	...
	(modifier-cond
	 ...
	 (modifier-cond
	  (and (pitem-attvec (unifiable (+att -pron)))
	       (not (pitem-following-word (or (str= ,) (str= "("))))))
	 ))
  (:head)
  (? :skip-word (str= ,)
     (NUM (dep-type num))
  ...
  (* (P (dep-type mod)
	(modifier-cond (pitem-attvec (not (defined (move)))))
	(modifiee-cond
...
;	       (undefine (move))))
	...
	(modifier-cond 
	 (or (pitem-attvec
	     ...
	     ;; infinitive clause
	     (and
	      (pitem-attvec (unifiable ((cform inf) +pro ~move)))
	     ...
	     (and (pitem-attvec (and (contain ((move (N (slash wh)))))
				     (unifiable (-bare))))
		  (not (pitem-preceding-word (str= ,)))
...
(def-modifiers Appo
  (:head
   (N (dep-type head)
      ...
      (modifier-cond
       (and
	(pitem-following-word (in (, -- --- _ __ ___ "(")))
	(pitem-attvec (and (unifiable (-appo)) (defined (lexitem)))))))
   )
  (? :skip-word (in (, -- --- _ __ ___ "("))
  ...
  (:skip-word (in (, -- --- _ __ ___ "("))
   (N (dep-type appo)
	...
	(or (pitem-following-word (in (. -- --- _ __ ___ ")" , ";" "\"" ?)))
	    (not (pitem-following-word)))
	(not (pitem-preceding-word (str= "\"")))))
      (merged-cond
       (prog
	(if (pitem-following-word (in (, -- --- _ __ ___ "(" ")")))
	    (include-following-word (in (, -- --- _ __ ___ "(" ")"))))
  ...
  (:minimal
     ...
     (Have (dep-type inv-have)
	   ...
	   (modifiee-cond
	     (pitem-attvec
	      (and 
	       (unifiable (+perf ~auxform ~tense) -pro)
...
(def-modifiers QBE
  (:head
   (Be (dep-type head)
       (modifier-cond (pitem-attvec (not (contain (+be +perf)))))
       )
   )
  ((N (dep-type subj)))
  (:skip-adjunct
		 ...
		 (not (contain ((auxform to)))))))
	      (modifier-cond 
	       (and (pitem-attvec (unifiable (~move)))
		    ...
		    (or (pitem-attvec (and (contain ((cat n) +adv) (unifiable (-wh)))))
...
(def-modifiers N_C
  ...
  (:skip-adjunct
   (C (dep-type comp1)
      (check-arg 0)
      (modifier-cond (pitem-attvec (unifiable (~move (cform fin inf)))))))
  )
...
(roots
 ...
 (Command
  (pitem-attvec (and (contain ((move ((slash np)))))
		     (not (contain ((move () ((slash wh))))))))
  )
 (Adjunct (t))  ;; 
 (N (t) (int 10))
...
(def-rules English-lexical-rules
  ...
  (license-PRO-of-inf
    (lexitem (att (contain ((auxform to)))))
    (assert
     (make-lexitem (low) (low (-1)) (avm syn (N +pro (check np) (node PRO)))))
    )
   ...
   (let (item)
     ...
     (cond 
      ((litem (att (or (contain (verb (args ((cat n)))))
		       (contain (verb (args ((cat c))))))))
       (make-np-move (litem))
       (when (litem (att (contain (verb -passive (move ((cat n) (nform norm))) (args ((cat n)))))))
	 (setq item (duplicate-lexitem (litem)))
	 (get-val item
		  (prog
		   (att (move-att args move))
		   (att (unify (-passive (move () ((slash wh))))))))
	 (assert (get-val item))
	 )
       )
...
(def-tree-rules deps-modifier
  ...
  (up (tree (cat (or (str= PRO) (str= EC))))
      (match
       (let (controller this antecedent)
