...
;(last-period
;  (seq (and-pat (word (reg-match ".*[.]$"))
;		(not-in-lexicon))
;       (* (word (or (reg-match [^a-zA-Z0-9#$]) (in skip.txt))))
;       (end-of-sentence))  
;  (split-token))
;
...
(novel-proper-nouns
  :precondition (is-mixed-case)
  (word (and (reg-match [A-Z]) (not (in (the an a) prep.txt prons.txt))))
...
(number
  ...
  (seq (? (or-pat (word (in (next other nearly roughly almost about over)))
	   ...
	   (* (word (in (- an a and to through))))))
       (word (or (in numbers.txt) (reg-match "[0-9][0-9.]*")))
       (constraint
	(and (or ($ 1 (matching-word))
		 ($ 3 (matching-word (not (in (an a))))))
	     )))
  (prog
...
(amount
  ...
  (seq (? (word (in (last next previous about past almost nearly over))))
       (+ (or-pat (lexitem (att (contain syn ((sem (+number))))))
		  (word (in (a an)))))
       (? (word (str= -)))
...
(date-misc1
  ...
  (prog
    (delete-if (low) (high) (pred (t)))
    (if ($ 2 (matching-word (reg-match "[0-9]+$")))
	(assert (make-lexitem (low) (high) (avm syn (PN -det +adv (sem (+date))))))
...
;	     (pred (att (not (contain syn ((cat v)))))))
  ...
  (when (not (or (matching-word
    ...
    (assert
	 ...
	 (nil)
      ...
      ((matching-word (or (suffix (sive ble ed ing al ic ive lar cent ous less))
...
(hyphen-unknown1
  ...
  (seq 
   (+ (seq
        (or-pat (lexitem (att (not (contain (Adv)))))
		(word (reg-match [A-Za-z0-9])))
        (word (str= -))
	(? (word (in (, and or))))))
   (word (not (in (a the an - and or , have had) skip.txt auxiliary.txt prons.txt)))
   )
    ...
    (let (first-word new-item)
      ...
      (setq new-item
	    (cond 
	     ...
	     (($ 2 (matching-word
		    (or (in (- about above across after along at between by down for in inside into of off on out over to up with without))
			(reg-match [A-Z0-9])
			(suffix (s)))))
...
(del-have
  ...
  (seq (lexitem (and (root (str= have))
		     (att (contain syn ((cat v))))))
       (word (and (not (reg-match un been))
		  (suffix (ed en)))))
  (delete ($ 1 (litem))))
...
(conj
 (seq (lexitem (att (contain syn (Tn))))
      ...
      (lexitem (att (contain syn (Tn)))))
 (let (new-item)
   (setq new-item (make-lexitem (low) (high) ($ 1 (meaning))))
   (add-components
    (get-val new-item) (low) (high) (pred (att (contain syn (Tn)))))))
...
(prep-followed-by-verb
 (lexitem (and (att (contain ((cat p))))
	       (root (not (in (to but before as "on to"))))))
 (del-following
  (pred
