...
(alias Tn.[pr]  ;;
  (make-av (verb (args (N -adv (role subj)) (N -adv (role obj)) (pp  +opt (role mod)))))
  )

(alias T[n.pr]  ;;
  (make-av (verb (args (N -adv (role subj)) (N -adv +opt (role obj)) (pp  +opt (role mod)))))
  )
...
(alias Tnpfin   ;;
  (make-av (verb (args (N -adv (role subj)) (N -adv (role obj)) (cp (cform fin) (role fc)))))
  )
...
(alias Cn.a   ;;
  (make-av (verb (rare very) (args (N -adv (role subj)) (N -adv (role obj)) ((cat a) -adv ~move (role desc)))))
  )

(alias Cn.n   ;;
  (make-av (verb (rare very) (args (N -adv (role subj)) (N -adv (role obj)) (N -adv (role desc)))))
  )
...
(alias Dn.n   ;;
  (make-av (verb (rare very) (args (N -adv (role subj)) (N -adv (role obj)) (N -adv (role obj2)))))
  )
...
(alias ItAftw ;;
  (make-av ((cat a) -adv -att +prd (args ((cat c) ~move (role subj) (cform fin inf)))))
  )
